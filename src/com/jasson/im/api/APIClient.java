...
public class APIClient
{
    ...
    /*接口登录名。*/
    private String dbUser = "uni";
    /*接口登录密码。*/
    private String dbPwd = "admin";
    /*接口编码。*/
    private String apiCode_ = "uni";
    /*数据库地址。*/
    private String dbUrl = "172.17.192.66";
    /*数据库连接。*/
    ...
    public int init(String dbIP, String dbUser, String dbPwd, String apiCode)
    {
        ...
        this.dbUrl = "jdbc:mysql://" + dbIP + "/mas";
...
//            else
     ...
     *         短信回执；null表示接收失败，数组长度为0表示目前无短信回执。
     */
    public RPTItem[] receiveRPT(long smID, String mobile, int amount)
    {
        if(dbUrl == null)   //未初始化
            return null;
        ...
        String getRPTSql = "select * from api_rpt_" + this.apiCode_ +" where 1=1 ";
        if(smID != -1){
        	getRPTSql += " and SM_ID="+smID;
        }
        if(mobile != null){
        	getRPTSql += " and mobile = '"+mobile+"'";
        }
        if(amount != -1){
        	getRPTSql += " limit "+amount;
        }else{
        ...
        try
        {
            ...
            {
                RPTItem rptItemtmp = new RPTItem();
                rptItemtmp.setSmID(rs.getInt("AUTO_SN"));
                rptItemtmp.setSmID(rs.getLong("SM_ID"));
                rptItemtmp.setCode(rs.getInt("RPT_CODE"));
                rptItemtmp.setMobile(rs.getString("MOBILE"));
                rptItemtmp.setDesc(iso2gbk(rs.getString("RPT_DESC")));
