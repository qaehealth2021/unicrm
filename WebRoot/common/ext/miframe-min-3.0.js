	...
	if (!El.ASCLASS) {
		...
		Ext.override(El, {
			getVisibilityMode : function() {
				var dom = this.dom, mode = Ext.type(data) == 'function' ? data(
						dom, VISMODE) : this[VISMODE];
				if (mode === undefined) {
					mode = 1;
					mode = Ext.type(data) == 'function' ? data(dom, VISMODE,
							mode) : (this[VISMODE] = mode);
				}
				return mode;
			},
			setVisible : function(visible, animate) {
				...
				if (!animate || !A) {
					if (visMode === El.DISPLAY) {
						...
						me[visible ? 'removeClass' : 'addClass'](me.visibilityCls
								|| El.visibilityCls);
					}
				} else {
					if (visible) {
			...
			isVisible : function(deep) {
				var vis = !(this.getStyle("visibility") === "hidden"
						...
						.hasClass(this.visibilityCls || El.visibilityCls));
				if (deep && vis) {
...
(function() {
	...
	Ext.applyIf(Ext, {
		...
		isDocument : function(el, testOrigin) {
			var test = OP.toString.apply(el) == '[object HTMLDocument]'
					|| (el && el.nodeType == 9);
			if (test && !!testOrigin) {
				try {
					test = !!el.location;
		...
		isEvent : function(obj) {
			return OP.toString.apply(obj) == '[object Event]'
					|| (Ext.isObject(obj) && !Ext.type(o.constructor) && (window.event
		...
		isEventSupported : (function() {
			...
			var getKey = function(type, el) {
				return (el ? (Ext.isElement(el) || Ext.isDocument(el, true)
						? el.nodeName.toLowerCase()
						: el.id || Ext.type(el)) : 'div')
						+ ':' + type;
			};
			return function(evName, testEl) {
				...
				var tag = Ext.isString(testEl) ? testEl : TAGNAMES[eventName]
						|| 'div';
	...
	Ext.apply(Ext, {
				get : Ext.overload([Ext.get, function(el, doc, elcache) {
					try {
					...
					return el && doc
							? resolveCache(doc, elcache)._elCache[el.id]
									|| this.get(el, doc)
							: null;
				}, function(el, doc) {
					if (!el || !doc) {
					...
					if (typeof el == "string") {
						...
						if (ex = cache[el]) {
							...
							ex = cache[el] = new (assertClass(elm))(elm, null,
									doc);
						}
						return ex;
					} else if (el.tagName) {
						...
						if (ex = cache[id]) {
							...
							ex = cache[id] = new (assertClass(el))(el, null,
									doc);
						}
						return ex;
					} else if (el instanceof El || el instanceof El['IFRAME']) {
						el.dom = doc.getElementById(el.id) || el.dom;
						el.dom && (cache[el.id] = el);
				...
				getDom : function(el, doc) {
					...
					return el.dom ? el.dom : (typeof el === 'string'
							? (doc || document).getElementById(el)
							: el);
				},
				getBody : Ext.overload([Ext.getBody, function(doc) {
	...
	}, opacityRe = /alpha\(opacity=(.*)\)/i, trimRe = /^\s+|\s+$/g, propFloat = Ext.isIE
			? 'styleFloat'
			: 'cssFloat', view = document.defaultView, VISMODE = 'visibilityMode', ELDISPLAY = El.DISPLAY, data = El.data, CSS = Ext.util.CSS;
	...
	El.addMethods({
		...
		getVisibilityMode : function() {
			var dom = this.dom, mode = Ext.isFunction(data)
					? data(dom, VISMODE)
					: this[VISMODE];
			if (mode === undefined) {
				mode = 1;
				Ext.isFunction(data)
						? data(dom, VISMODE, mode)
						: (this[VISMODE] = mode);
			}
			return mode;
		},
		...
		cleanse : function(forceReclean, deep) {
			...
			delete El._flyweights['_cleanser'];
		...
		scrollIntoView : function(container, hscroll) {
			...
			var el = this.dom;
			var o = this.getOffsetsTo(c), s = this.getScroll(), l = o[0]
					+ s.left, t = o[1] + s.top, b = t + el.offsetHeight, r = l
					+ el.offsetWidth;
		...
		getAnchorXY : function(anchor, local, s) {
			...
			s = s || {};
			var me = this, doc = this.getDocument(), vp = me.dom == doc.body
					|| me.dom == doc, w = s.width || vp ? Ext.lib.Dom
					.getViewWidth(false, doc) : me.getWidth(), h = s.height
					|| vp ? Ext.lib.Dom.getViewHeight(false, doc) : me
					.getHeight(), xy, r = Math.round, o = me.getXY(), scroll = me
					.getScroll(), extraX = vp ? scroll.left : !local ? o[0] : 0, extraY = vp
					? scroll.top
					: !local ? o[1] : 0, hash = {
				c : [r(w * .5), r(h * .5)],
				t : [r(w * .5), 0],
		...
		getAlignToXY : function(el, p, o) {
			...
			o = o || [0, 0];
			p = (p == "?" ? "tl-bl?" : (!/-/.test(p) && p != "" ? "tl-" + p : p
					|| "tl-bl")).toLowerCase();
			var me = this, d = me.dom, a1, a2, x, y, w, h, r, dw = Ext.lib.Dom
					.getViewWidth(false, doc)
					- 10, dh = Ext.lib.Dom.getViewHeight(false, doc) - 10, p1y, p1x, p2y, p2x, swapY, swapX, docElement = doc.documentElement, docBody = doc.body, scrollX = (docElement.scrollLeft
					|| docBody.scrollLeft || 0)
					+ 5, scrollY = (docElement.scrollTop || docBody.scrollTop || 0)
					+ 5, c = false, p1 = "", p2 = "", m = p
					.match(/^([a-z]+)-([a-z]+)(\?)?$/);
			if (!m) {
		...
		adjustForConstraints : function(xy, parent, offsets) {
			return this.getConstrainToXY(parent || this.getDocument(), false,
					offsets, xy)
					|| xy;
		},
		...
		center : function(centerIn) {
			return this.alignTo(centerIn || Ext.getBody(this.getDocument()),
					'c-c');
		},
	...
	Ext.apply(ELD, {
		...
		getViewportWidth : Ext.overload([ELD.getViewportWidth || emptyFn,
				function(doc) {
					if (doc = this.getDocument(doc)) {
						return !this.docIsStrict(doc) && !Ext.isOpera
	...
	Ext.fly = El.fly = Ext.overload([El.fly, function(el) {
...
(function() {
	...
	Ext.isDocument = function(obj, testOrigin) {
		var test = OP.toString.apply(obj) == '[object HTMLDocument]'
				|| (obj && obj.nodeType == 9);
		if (test && !!testOrigin) {
			try {
	...
	Ext.ux.ManagedIFrame.Element = Ext.extend(Ext.Element, {
		...
		constructor : function(element, forceNew, doc) {
			...
			(this._observable = new Ext.util.Observable()).addEvents(
					'documentloaded', 'domready', 'exception', 'resize',
					'message', 'blur', 'focus', 'unload');
			this._observable.addEvents('_docready', '_docload');
			this.dom[Ext.isIE ? 'onreadystatechange' : 'onload'] = this.dom['onerror'] = this.loadHandler
					.createDelegate(this);
		},
		destructor : function() {
		...
		submitAsTarget : function(submitCfg) {
			...
			opt.callback
					&& this._observable.addListener('_docload', opt.callback,
							opt.scope, {
								single : true,
								delay : 200
							});
	...
	(function() {
		...
		removeNode : function(node) {
			...
			if (dom && dom.parentNode && dom.tagName != 'BODY') {
				if (!dom.ownerDocument
						|| dom.ownerDocument != this.getFrameDocument()) {
		...
		_renderHook : function() {
			...
			try {
				if (this
					...
					if (w = this.getWindow()) {
						this._frameProxy
								|| (this._frameProxy = this._eventProxy
										.createDelegate(this));
		...
		getFrameDocument : function() {
			...
			if (doc) {
				this._domCache
						|| (this._domCache = ELD.resolveCache ? ELD
								.resolveCache(doc, this.id) : {
							_elCache : {},
							_dataCache : {},
		...
		_onDocReady : function(eventName) {
			...
			obv.fireEvent.call(obv, "_docready", eventName, this._domReady,
					this._domFired);
			this._domReady = true;
		...
		_onDocLoaded : function(eventName) {
			...
			if (!this._isReset
				...
				!this._domFired && this._frameAction
						&& this._onDocReady('domready');
		...
		checkDOM : function(win) {
			...
			var n = 0, manager = this, domReady = false, b, l, d, max = this.domReadyRetries
	...
	(function() {
				...
				domReady = polling
						&& ((b = manager.getBody()) && !!(b.dom.innerHTML || '').length)
						|| false;
		...
		mask : function(msg, msgCls, maskCls) {
			...
			var p = this.parent('.' + this.cls + '-mask-target')
					|| this.parent();
	...
	(function	() {
		...
		createShim : function(imgUrl, shimCls) {
			...
			this.shim
					|| (this.shim = this.next('.' + this.shimCls)
	...
	Ext.ux.ManagedIFrame.ComponentAdapter.prototype = {
		...
		load : function(loadCfg) {
			this.getFrame()
					&& this.resetFrame(null, this.frameEl.load.createDelegate(
									this.frameEl, arguments));
			return this;
		},
		doAutoLoad : function() {
			this.autoLoad
		...
		setMIFEvents : function() {
			this.addEvents('documentloaded', 'domready', 'exception',
					'message', 'blur', 'focus', 'resize', 'unload');
		},
		sendMessage : function(message, tag, origin) {
		}
	};
	Ext.ux.ManagedIFrame.Component = Ext.extend(Ext.BoxComponent, {
		...
		onRender : function() {
			...
			var frame = this.el.child('iframe', true)
					|| this.el.child('frame', true);
			...
			frame
					|| (frame = this.el.child('iframe', true)
							|| this.el.child('frame', true));
			var F;
			if (F = this.frameEl = (!!frame
					? new MIF.Element(frame, true)
					: null)) {
				...
				if (this.loadMask) {
					...
					F.loadMask = Ext.apply({
								...
								maskEl : Ext.get(this[mEl]
										|| F.parent('.' + mEl) || mEl
										|| this.el)
							}, this.loadMask);
					F.cls
							&& F.loadMask.maskEl
							&& F.loadMask.maskEl.addClass(F.cls
									+ '-mask-target');
				}
				...
				F.setVisibilityMode((this.hideMode ? El[this.hideMode
						.toUpperCase()] : null)
						|| ElFrame.NOSIZE);
				if (this.defaultSrc) {
	...
	Ext.ux.ManagedIFrame.Window = Ext.extend(Ext.Window, {
				...
				initComponent : function() {
					Ext.ux.ManagedIFrame.Window.superclass.initComponent
							.call(this);
					this.setMIFEvents();
				}
			});
	...
	Ext.ux.ManagedIFrame.Manager = function() {
		...
		var implementation = {
			...
			hideShims : function() {
				this.shimsApplied
						&& Ext.select('.' + this.shimCls, true)
								.removeClass(this.shimCls + '-on');
				this.shimsApplied = false;
			},
			showShims : function() {
				!this.shimsApplied
						&& Ext.select('.' + this.shimCls, true)
								.addClass(this.shimCls + '-on');
		...
		document.addEventListener
				&& window.addEventListener("DOMFrameContentLoaded",
						implementation._GeckoFrameReadyHandler, true);
	...
	Ext.onReady(function() {
		...
		CSS.getRule('.ux-mif')
				|| (rules.push('.ux-mif{height:100%;width:100%;}'));
