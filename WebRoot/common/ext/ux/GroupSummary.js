...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
        ...
        for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
            ...
            if (cf.summaryType) {
                for (var j = 0, jlen = rs.length; j < jlen; j++) {
                    ...
                    if(cf.sumAfterRenderer){
                    	var val=cf.renderer(r.get(cname),{},r);
                   		data[cname] = Ext.ux.grid.GridSummary.Calculations[cf.summaryType](val, r, cname, data, j);
                    }else{
	...
	getSumTypeValue: function(dataIndex){
		var g = this.grid, ds = g.store,
            cs = this.view.getColumnData(),
            cm = this.cm,
            rs = ds.getRange(),
...
Ext.ux.grid.GridSummary.Calculations = {
    sum : function(v, record, colName, data, rowIdx) {
        return data[colName].add(Ext.num(v, 0)); //+ Ext.num(v, 0);
    },
    ...
    average : function(v, record, colName, data, rowIdx) {
        var t = data[colName].add(Ext.num(v, 0)), count = record.store.getCount();
